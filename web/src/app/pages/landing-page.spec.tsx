...
describe("LandingPage", () => {
  it("has one H1 and the three role-specific signup targets", () => {
    ...
    expect(screen.getByRole("link", { name: /Order food/i })).toHaveAttribute(
      "href",
      "/signup?role=CUSTOMER",
    );
    expect(screen.getByRole("link", { name: /Add your restaurant/i })).toHaveAttribute(
      "href",
      "/signup?role=OWNER",
    );
    expect(screen.getByRole("link", { name: /Deliver with us/i })).toHaveAttribute(
      "href",
      "/signup?role=COURIER",
    );
  });
